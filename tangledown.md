...
I just described a kind of [Literate Programming](https://en.wikipedia.org/wiki/Literate_programming), a method of software documentation invented by [Donald Knuth](http://amturing.acm.org/award_winners/knuth_1013846.cfm). Because our documentation language is Markdown, we'll call this technique _Literate Markdown_. 
...
This tool pulls or _tangles_ code out of any Markdown document. The verb "tangle" is traditional in the literate-programming community. You might think it really means "untangle," because the order of presentation in the Markdown document is all tangled up from the point of view of the build tools. But Knuth thinks it's the other way around: he prefers the human's point of view. The build tools need the code all tangled up in some other, essentially arbitrary order.

If we need something _more_ powerful than Markdown, say to produce publishable PDF files, we'll go back to org-mode and org-babel in emacs, which are best-of-breed, classical literate programming tools. But Markdown is good enough for Github, and thus for the vast majority of open-source code in the world.

## Exploit: MOU Ignores Mysterious Tags

Let's exploit the fact that markdown renderers like [MOU](http://25.io/mou/) ignore HTML / XML tags that they don't recognize. We can name blocks of code with `noweb` tags, like this:
...
Leave the blank line after the opening `<noweb...>` tag and before the ending `</noweb>` tag, unless you want MOU not to render the code like code. Here's a non-rendered `noweb` block --- no blank lines surrounding the contents of the noweb tag:
...
But this here Tangledown compiler _doesn't_ ignore the tags. Tangledown is a Python script that sucks up the contents of the `noweb` tag pairs and sticks them into a dictionary with the keys `my_little_tests` or `another_little_test` for the samples above. In general, the key for a noweb block is the name in the `name` attribute of the `noweb` tag. Later, Tangledown will blow those lines back out wherever it sees a `block` tag with the same name. 

Often, that `block` tag will be inside a `tangle` tag that causes its entire contents to spray out to the disk drive to a file path named in the `file` attribute of the `tangle` tag. The same rules for blank lines hold for `tangle` tags as they do for `noweb` tags: if you want MOU to render the contents like code, bracket the contents with blank lines; otherwise, leave the blank lines out. The following
...
Because the `block` tag is inside a top-level `tangle` or `noweb` tag, MOU will render the `block` tag verbatim to the documentation. This is OK for humans, who will think "AHA!, this bit of code refers to some other code that I should read in another place and time. This here beautifully written document will make it easy for mea to understand the big picture by breaking things up like this." 

In fact, that's exactly what you want for humans: talk about something in a place where you don't necessarily implement it.  But compilers need the block _right here and now_. Tangledown will substitute the contents of a block for the block tag in a tangle block when writing files to the disk for you. The substitution process is hidden, behind the scenes, and only needed when it's time to create documents on your hard drive. 

You don't need any contents in a block tag, but you're welcome to put some in. Tangledown will eat and ignore any contents in a `block` tag.

## Bootstrapping Step-by-Step

This here document that you're reading right now is literature, after all, so we should tell a story. Let's tell the story of creating Tangledown. And we'll use the tools we're inventing to _build_ the tools we're inventing. That's just like bootstrapping a compiler. Tangledown is a (very small) compiler. Just as we ordinarily use a compiler to compile a compiler, we'll use Tangledown to tangle Tangledown itself out of this here document you're reading right now.
...
The following regular expressions match tags that may appear anywhere on a line. I was surprised to find that I needed the leading '.*'. I don't know the reason. Also, I converted the 'o' in 'block' to a harmless group '[o]' so that _block_end_ doesn't match itself. That makes it safe to run this code on this here document itself.
...
The following code has two block tags that refer to the noweb tags of regular expressions defined above. After Tangledown substitutes the contents of the noweb tags, the code becomes valid Python and you can run it. When you run it, it proves that we can regognize all the various kinds of tags. Notice the special treatment for block ends, which will usually be on the same lines as their block tags, but not necessarily so.
...
Tangledown operates in two passes, once over the file and a second time over the tangle blocks that it collected in the first pass. In the second pass, Tangledown does the substitution for block tags, creating valid code on the disk.
...
Tangledown is a little compiler. We could go  highfalutin' and write it in
state-machine and parser-combinator style, and then it would be very pretty and
easy to explain to a compiler theorist. However, we want to make it

- very short
- independent of rich libraries like parser combinators
- completely obvious to anyone

We'll just use global variables and side effects, but in a tasteful way.

The function `accumulate` starts at line `i`, figures out whether a tag's first
non-blank line is triple backtick, in which case it _won't_ snip four spaces
from the beginning of every line, and keeps going until it sees then end of the
tag. This function is meant to accumulate the contents of noweb or tangle tags.
...
The next bit of code is top-level script. It sucks all the noweb tags and tangle
tags out of a file, but doesn't expand any block tags that it finds. We use it just to build up our internal dictionaries.
...
too many or not enough triple-backtick lines, broken block tags, and more. 

We'll get to error handling someday, maybe. Tangledown is just a little tool for
personal use at the moment. If it's going to be distributed to hostile users,
then we will enumerate all the bad cases and handle them. 

But not now. Let's get the happy case right, first. 



### Second Pass: Expanding Blocks

Now, we're going to iterate over all the tangle tag contents and expand the
block tags we find in there. We must keep going until there are no more block
tags, because noweb blocks are allowed (encouraged!) to refer to other blocks.
If there are cycles, this will hang. DUDE! We're doing the happy case first, and
...
The following function does one round of block expansion. The caller will check
...
As we said, lots of things can go wrong. Maybe someday we'll enumerate them all
